...
void statement() {
    ...
    if (cur_token.type == TK_WHILE) {
        next_token(); next_token();              // 跳過 'while' 和 '('
        int loop_start = quad_count;             // 記住條件判斷的起始位置
        char cond[32]; expression(cond);
        next_token(); next_token();              // 跳過 ')' 和 '{'
        int jmp_idx = quad_count;                // 記下 JMP_F 位置，等迴圈結束後回填
        emit("JMP_F", cond, "-", "?");
        while (cur_token.type != TK_RBRACE) statement();
        char loop_start_str[16]; sprintf(loop_start_str, "%d", loop_start);
        emit("JMP", "-", "-", loop_start_str);  // 無條件跳回條件判斷
        next_token();                            // 跳過 '}'
        sprintf(quads[jmp_idx].result, "%d", quad_count); // 回填跳出位置
